...
-- Remove attack animations toggle
local switch = KillingTab:AddSwitch("Remove Attack Animations", function(enabled)
	local blockedAnimations = {
		["rbxassetid://3638729053"] = true,
		["rbxassetid://3638767427"] = true,
	}

	local function stopBlockedTracks(humanoid)
	...
	local function setupAnimationBlocking()
		...
		stopBlockedTracks(humanoid)
		_G.AnimBlockConnection = humanoid.AnimationPlayed:Connect(stopBlockedTracks)
	end
			...
			if not tool:GetAttribute("ActivatedOverride") then
				...
				_G.ToolConnections[tool] = tool.Activated:Connect(function()
					...
					if char and char:FindFirstChild("Humanoid") then
						stopBlockedTracks(char.Humanoid)
					end
				end)
			end
		end
	...
	if enabled then
		...
		-- Disconnect all connections and remove overrides
		for _, conn in pairs({_G.AnimBlockConnection, _G.AnimMonitorConnection, _G.CharacterAddedConnection, _G.BackpackAddedConnection, _G.CharacterToolAddedConnection}) do
		...
		if _G.ToolConnections then
			for tool, conn in pairs(_G.ToolConnections) do
				if conn then conn:Disconnect() end
...
local killSwitch = KillingTab:AddSwitch("Kill Everyone", function(bool)
	...
	if bool and not _G.killAllConnection then
	...
	elseif not bool and _G.killAllConnection then
...
local friendsSwitch = KillingTab:AddSwitch("Whitelist Friends", function(bool)
	_G.whitelistFriends = bool

	local function addFriendToWhitelist(player)
	...
	if bool then
		...
		-- Add friends who join later
		game.Players.PlayerAdded:Connect(function(p)
			if _G.whitelistFriends and p:IsFriendsWith(player.UserId) then
				addFriendToWhitelist(p)
			end
		end)
	end
end)
friendsSwitch:Set(false)
...
-- Populate whitelist and blacklist dropdowns
for _, p in ipairs(game.Players:GetPlayers()) do
	if p ~= game.Players.LocalPlayer then
		local displayText = getPlayerDisplayText(p)
...
local blacklistKillSwitch = KillingTab:AddSwitch("Kill List", function(bool)
	...
	if bool and not _G.blacklistKillConnection then
		_G.blacklistKillConnection = game:GetService("RunService").Heartbeat:Connect(function()
			if _G.killBlacklistedOnly then
				for _, p in ipairs(game.Players:GetPlayers()) do
					if p ~= game.Players.LocalPlayer and isBlacklisted(p) then
						killPlayer(p)
					end
				end
			end
		end)
	elseif not bool and _G.blacklistKillConnection then
...
local specdropdown = KillingTab:AddDropdown("Choose Player", function(text)
	for _, p in ipairs(updatePlayerList()) do
		local optionText = getPlayerDisplayText(p)
		if text == optionText then
			selectedPlayerToSpectate = p
			if spectating then
				updateSpectateTarget(p)
			end
			break
		end
	end
...
-- Populate spectate dropdown
for _, p in ipairs(updatePlayerList()) do
	specdropdown:Add(getPlayerDisplayText(p))
end
...
game.Players.PlayerRemoving:Connect(function(p)
	if selectedPlayerToSpectate and selectedPlayerToSpectate == p then
...
local deathRingSwitch = KillingTab:AddSwitch("Death Ring", function(bool)
	...
	if bool and not _G.deathRingConnection then
		_G.deathRingConnection = game:GetService("RunService").Heartbeat:Connect(function()
			...
			for _, p in ipairs(game:GetService("Players"):GetPlayers()) do
				if p ~= game.Players.LocalPlayer and not isWhitelisted(p) and isPlayerAlive(p) then
	...
	elseif not bool and _G.deathRingConnection then
